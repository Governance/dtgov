...
/**
 * An authentication provider that uses SAML Bearer Tokens. The Dtgov API must
 * be configured to accept and consume a SAML Assertion. For more information
 * see {@link SAMLBearerTokenLoginModule}.
 * 
...
public class SAMLBearerTokenAuthenticationProvider implements AuthenticationProvider {
    ...
    private String createSAMLBearerTokenAssertion() {
        String issuer = config.getString(DtgovUIConfig.DTGOV_API_SAML_ISSUER);
        String service = config.getString(DtgovUIConfig.DTGOV_API_SAML_SERVICE);
        String samlAssertion = SAMLAssertionUtil.createSAMLAssertion(issuer, service);
        boolean signAssertion = "true".equals(config.getString(DtgovUIConfig.DTGOV_API_SAML_AUTH_SIGN_ASSERTIONS)); //$NON-NLS-1$
        if (signAssertion) {
            String keystorePath = config.getString(DtgovUIConfig.DTGOV_API_SAML_AUTH_KEYSTORE);
            String keystorePassword = config.getString(DtgovUIConfig.DTGOV_API_SAML_AUTH_KEYSTORE_PASSWORD);
            String keyAlias = config.getString(DtgovUIConfig.DTGOV_API_SAML_AUTH_KEY_ALIAS);
            String keyAliasPassword = config.getString(DtgovUIConfig.DTGOV_API_SAML_AUTH_KEY_PASSWORD);
            try {
