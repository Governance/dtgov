...
public class QueryAccesor {
    ...
    /** The logger. */
    private static Logger logger = LoggerFactory.getLogger(QueryAccesor.class);
    ...
    public Set<Query> getQueries() throws ConfigException {
        ...
        query.propertyName(PROPERTY_WORKLOW);
        query.propertyName(PROPERTY_QUERY);
        SrampClientQuery scq = query.startIndex(0);
        try {
            QueryResultSet resultSet = scq.query();
            for (ArtifactSummary artifactSummary : resultSet) {
                String srampQuery = artifactSummary.getCustomPropertyValue(PROPERTY_QUERY);
                String workflowId = artifactSummary.getCustomPropertyValue(PROPERTY_WORKLOW);
                Map<String, String> parameters = artifactSummary.getPropertiesByPrefix(PROPERTY_PREFIX);

                Query q = new Query(srampQuery, workflowId);
                int i = 0;
                for (String paramKey : parameters.keySet()) {
                    if (StringUtils.isNotBlank(paramKey)) {
                        String value = parameters.get(paramKey);
                        q.addParameter(paramKey, value);
                    }
                }
