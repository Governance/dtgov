  ...
  <xsl:template match="/*[name()='server' or name()='domain']//*[name()='profile']/od:subsystem/od:deployments">
    ...
    <xsl:element name="deployments" namespace="{$currentNS}">
      ...
      <xsl:element name="deployment" namespace="{$currentNS}">
        <xsl:attribute name="name">dtgov.war</xsl:attribute>
        <xsl:attribute name="module">org.overlord.dtgov.eap.deployments</xsl:attribute>
      </xsl:element>
      <xsl:element name="deployment" namespace="{$currentNS}">
        <xsl:attribute name="name">dtgov-ui.war</xsl:attribute>
        <xsl:attribute name="module">org.overlord.dtgov.eap.deployments</xsl:attribute>
      </xsl:element>
      <xsl:element name="deployment" namespace="{$currentNS}">
        <xsl:attribute name="name">dtgov-ds.xml</xsl:attribute>
        <xsl:attribute name="module">org.overlord.dtgov.eap.deployments</xsl:attribute>
      </xsl:element>
    </xsl:element>
  </xsl:template>
